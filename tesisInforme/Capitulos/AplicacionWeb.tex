...
Después de completar los ciclos ascendente y descendente de medición, se dispone de un conjunto de datos crudos de velocidad y dirección del viento. El algoritmo lee estos datos y extrae las mediciones de la parte plana estable (como se ilustra en la Figura \ref{fig:curvaEscalon} de la sección siguiente) aplicando el mismo procedimiento a cada segmento. Este proceso se denomina \texttt{Parsear los datos}. Posteriormente, se muestran los datos crudos procesados en pantalla, donde el operador, desde el \textit{front end}, puede iniciar el cálculo de incertidumbre, siempre que considere que las muestras son útiles. En caso contrario, puede reiniciar las mediciones y obtener un nuevo perfil. Si los datos son correctos para la calibración, se calcula el promedio $\bar{v}$ y el desvío estándar $\sigma_{v}$ de las mediciones de velocidad del viento para cada valor configurado. A continuación, se elabora el presupuesto de incertidumbre, obteniendo los datos de histéresis del patrón y del IBC a partir de la diferencia de los valores promedio. Se procede con la lectura de los metadatos ingresados por el operador, específicamente la resolución de los instrumentos y el área de bloqueo de los anemómetros para el cálculo del factor de bloqueo, ecuación \ref{eq:FactorDeBloqueo}, donde $S_{T}$ corresponde al área transversal de la zona de medición del túnel de viento y $S_{A}$ se corresponde al área efectiva de bloqueo del sensor y su soporte. Este valor se utiliza para corregir las mediciones del IBC y del patrón, y se agrega al presupuesto su respectiva incertidumbre según la ecuación \ref{eq:incertiFactorBloqueo}.
...
\end{equation}
Luego, se obtienen los datos de calibración del sensor patrón a partir de un ajuste lineal de los puntos discretos del certificado. En esa curva de ajuste obtenida, se evalúa los valores promedio medidos de patron $\bar{v}_{Pat}$. Esto se hace así porque no siempre el patrón está calibrado en los mismos puntos en los que se lo va a utilizar. Lo mismo se realiza con el certificado del túnel, se extrae los valores de estabilidad, homogeneidad y el factor de calibración del túnel para los los valores promedio medidos del IBC $\bar{v}_{IBC}$. Con todas estas incertidumbres normalizadas y multiplicadas por su coeficiente de sensibilidad $c_{i}$, se obtiene la incertidumbre estándar. Luego, se realiza la suma de los cuadrados, ecuación \ref{eq:incertCombinadaSoft}, de cada incertidumbre estándar y se calcula la raíz cuadrada para obtener la incertidumbre combinada para cada punto configurado por el operador.
\begin{equation}
    \centering
...
El \textit{front end} se encarga de:
\begin{itemize}
    \item Interacción del usuario a través de formularios, botones y eventos.
    \item Comunicación con el \textit{back end} a través de Django y \textit{WebSockets}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\linewidth]{Figuras/AplicacionWeb/frontend/index.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\linewidth]{Figuras/AplicacionWeb/frontend/procesarDatos1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Figuras/AplicacionWeb/frontend/curvaHisteris.png}
