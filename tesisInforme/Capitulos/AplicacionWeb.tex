...
(Hacer referencia a la imagen de objetivos donde queda claro donde se ejecuta la aplicacion web , en el servidor)
...
Finalmente, la quinta capa consiste en un servidor \textit{WebSocket} que se conecta con un sistema embebido. Este servidor permite la comunicación bidireccional en tiempo real entre la aplicación y dispositivos embebidos, facilitando el control y monitoreo remoto. Esta capa es esencial para aplicaciones que requieren actualizaciones en tiempo real y una interacción continua con dispositivos de hardware.
(Mencionar que websocket del lado del server protocolo de comunicacion a nivel de aplicacion basado en http, como se detallo en el capitulo 4)
\begin{figure}[H]
    \centering
...
PAsar esta imagen al apendice y poner una diagrama de bloques,
se ve como se relacionan las tablas de la base de datos, que se detallan en el apendice tanto, se ve el bloque tanto que guarda tales datos. pongo lo que guarda esa tabla a grandes rasgos
\begin{figure}[H]
    \centering
